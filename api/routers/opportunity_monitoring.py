"""Opportunity watch, refresh, and change-history API routes."""
...
async def watch_opportunity(opportunity_id: str):
    ...
    try:
        ...
        # Activation is durable even when the first source check fails. Return the
        # stored monitor health so the UI can show the exact problem without
        # incorrectly telling the user that Watch was not enabled.
        try:
        ...
        logger.error(f"Failed to activate opportunity monitoring: {exc}")
        raise HTTPException(status_code=500, detail="Failed to watch opportunity")
...
async def unwatch_opportunity(opportunity_id: str):
    try:
        ...
        logger.error(f"Failed to disable opportunity monitoring: {exc}")
...
async def check_opportunity_now(opportunity_id: str):
    try:
        ...
        logger.error(f"Failed to refresh opportunity {opportunity_id}: {exc}")
        raise HTTPException(status_code=500, detail="Failed to refresh opportunity")
...
async def get_opportunity_changes(
    ...
    try:
        ...
        logger.error(f"Failed to list changes for {opportunity_id}: {exc}")
        raise HTTPException(status_code=500, detail="Failed to fetch change history")
...
async def acknowledge_changes(opportunity_id: str):
    try:
        ...
        logger.error(f"Failed to acknowledge changes for {opportunity_id}: {exc}")
        raise HTTPException(status_code=500, detail="Failed to acknowledge changes")


@router.post("/opportunities/monitoring/run-due", response_model=Dict[str, int])
async def run_due_opportunity_monitors():
    """Operational endpoint for manually running the same durable scheduler pass."""

    try:
        return await run_due_monitors_once()
    except Exception as exc:
        logger.error(f"Failed to run due opportunity monitors: {exc}")
        raise HTTPException(status_code=500, detail="Failed to run opportunity monitors")
