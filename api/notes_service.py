...
class NotesService:
    ...
    def get_all_notes(self, project_id: Optional[str] = None) -> List[Note]:
        """Get all notes with optional Project filtering."""
        notes_data = api_client.get_notes(project_id=project_id)
    ...
    def create_note(
        ...
        note_response = api_client.create_note(
            content=content, title=title, note_type=note_type, project_id=project_id
        )
        note_data = (
            note_response if isinstance(note_response, dict) else note_response[0]
        )
