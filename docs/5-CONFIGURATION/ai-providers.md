...
Construction OS uses a **credential-based system** for managing AI providers:
...
5. **Start using** the provider in your projects

> **Prerequisite**: You must set `CONSTRUCTION_OS_ENCRYPTION_KEY` in your docker-compose.yml before storing credentials. See [API Configuration](../3-USER-GUIDE/api-configuration.md#encryption-setup) for details.
...
**Available Models (in Construction OS):**
...
- Very cheap
- Great for artifacts/batch work
...
By default, Construction OS uses the LLM provider's embeddings. Embedding models are discovered and registered through the same credential system — when you discover models from a credential, embedding models are included alongside language models.
...
3. **Set `CONSTRUCTION_OS_ENCRYPTION_KEY`** in your docker-compose.yml (required for storing credentials)
