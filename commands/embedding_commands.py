...
async def legacy_embed_single_item_command(
    ...
    New code submits embed_source or embed_note directly. This
...
@command("vectorize_source", app="construction_os", retry=None)
...
async def collect_items_for_rebuild(
    ...
    Returns:
        Dict with keys: 'sources', 'notes' containing lists of item IDs
    """
    items: Dict[str, List[str]] = {"sources": [], "notes": []}
...
@command("rebuild_embeddings", app="construction_os", retry=None)
async def rebuild_embeddings_command(
    input_data: RebuildEmbeddingsInput,
) -> RebuildEmbeddingsOutput:
    """
    Rebuild embeddings for sources and/or notes.
    ...
    try:
        ...
        processing_time = time.time() - start_time
        jobs_submitted = sources_submitted + notes_submitted
