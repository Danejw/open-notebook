...
Source: Auglet `rag-quality-audit-prompt-set` (asset `75870325-9446-4a13-a283-456ffc575f52`).

## Project notes

- Working directory: repo root (`construction-os`)
- Write the completed report to `docs/audits/rag-quality-audit.md`
...
Act as a senior software architect and specialist auditor for **rag quality**.
...
Measure ingestion, parsing, chunking, metadata, embedding, indexing, retrieval, reranking, grounding, citation, freshness, and answer behavior to identify why a retrieval-augmented generation system may return incomplete, stale, unsupported, or irrelevant results.
...
- indexing and update behavior
- hybrid, vector, graph, and keyword retrieval
- filters and access control
- reranking and context assembly
- citation generation
- answer grounding and abstention
- evaluation datasets and telemetry
...
1. Map the complete RAG pipeline from source ingestion through answer generation.
2. Inspect representative documents, chunks, metadata, embeddings, index records, and deletion/update paths.
3. Evaluate retrieval on realistic questions including exact, semantic, relational, and negative cases.
4. Trace answer claims back to retrieved evidence and citation construction.
5. Separate retrieval failure, context assembly failure, and generation failure.
...
Calculate a single **RAG Quality Score from 0 to 100**.
...
|---|---:|---|
| Ingestion and Representation Quality | 20 | Parsing, chunking, structure, metadata, and source fidelity |
| Index and Freshness Reliability | 15 | Completeness, updates, deletions, deduplication, and model consistency |
| Retrieval Relevance and Coverage | 25 | Recall, precision, filtering, hybrid search, and reranking |
| Grounding and Citation Accuracy | 20 | Support for claims, correct provenance, and abstention |
| Access Control and Data Isolation | 10 | Permission-aware ingestion and retrieval |
| Evaluation and Operations | 10 | Datasets, metrics, tracing, monitoring, and regression controls |
| **Total** | **100** | |
...
`docs/audits/rag-quality-audit.md`
...
- **Issue type:** ingestion loss, chunking issue, metadata gap, stale index, duplicate chunk, retrieval miss, irrelevant retrieval, reranking issue, citation error, unsupported answer, access-control risk
...
- Do not judge RAG quality from a small set of anecdotal queries alone.
- Do not increase chunk size or top-k without considering precision, cost, and context limits.
- Treat citation presence and citation correctness as separate properties.
- Require permission filters to apply before sensitive content reaches the model.
- Do not silently answer when evidence is insufficient; evaluate abstention behavior.
