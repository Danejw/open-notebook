...
Use any server that implements the OpenAI API format with Construction OS. This includes LM Studio, Text Generation WebUI, vLLM, and many others.
...
Construction OS can connect to any server using this format.
...
### Step 3: Add Model in Construction OS
...
When Construction OS runs in Docker and your compatible server runs on the host, use the appropriate base URL when adding your credential in **Settings → API Keys**:
...
## Adding Models in Construction OS
...
```bash
docker exec -it construction-os curl http://host.docker.internal:1234/v1/models
```
...
4. Update model name in Construction OS
```
