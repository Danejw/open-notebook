...
Ollama provides free, local AI models that run on your own hardware. This guide covers everything you need to know about setting up Ollama with Construction OS, including different deployment scenarios and network configurations.
...
When both Construction OS and Ollama run directly on your machine:
...
### Scenario 2: Construction OS in Docker, Ollama on Host

When Construction OS runs in Docker but Ollama runs on your host machine:
...
When both Construction OS and Ollama run in the same Docker Compose stack:
...
# https://github.com/lfnovo/construction-os/blob/main/docker-compose.yml
services:
  construction-os:
    image: lfnovo/construction-os:v1-latest
    pull_policy: always
...
This is the most common cause of "Failed to send message" errors. Construction OS requires the **exact model name** as it appears in Ollama.
...
**Step 2: Use the exact name when adding the model in Construction OS**
...
**Note:** Some models use `:latest` as the default tag. If `ollama list` shows `model:latest`, you must use `model:latest` in Construction OS, not just `model`.

**Step 3: Configure in Construction OS**
...
6. Save the model
7. Set it as the default for the appropriate task (chat, artifact, etc.)

### Common Issues

**1. "Ollama unavailable" in Construction OS**
...
```bash
# If Construction OS runs in Docker or on a different machine,
# Ollama must bind to all interfaces, not just localhost
export OLLAMA_HOST=0.0.0.0:11434
ollama serve
```
> **Why this is needed:** By default, Ollama only accepts connections from `localhost` (127.0.0.1). When Construction OS runs in Docker or on a different machine, it can't reach Ollama unless you configure `OLLAMA_HOST=0.0.0.0:11434` to accept external connections.
...
**From inside Construction OS container, test Ollama:**
```bash
# Get into container
docker exec -it construction-os bash
...
1. **Model name mismatch**: The model name in Construction OS doesn't exactly match `ollama list`
2. **No default model configured**: You haven't set a default chat model in Settings → Models
3. **Model was deleted**: You removed the model from Ollama but didn't update Construction OS's defaults
4. **Model record deleted**: The model was removed from Construction OS but is still set as default
...
# Compare with what's configured in Construction OS
...
1. Refresh the Construction OS page
...
docker run --network host lfnovo/construction-os:v1-latest  # for quick testing only
```
...
# Construction OS configuration (configure via Settings → API Keys instead)
...
- [Construction OS Discord](https://discord.gg/37XJPXfz2w) - Integration help
...
This comprehensive guide should help you successfully deploy and optimize Ollama with Construction OS. Start with the Quick Start section and refer to specific scenarios as needed.