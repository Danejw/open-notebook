...
export default function CollectionDetailPage() {
  ...
  return (
    <>
      <div className="flex-1 overflow-y-auto">
        <div className={pageContentClassName}>
          <div className={cn('max-w-5xl', pageSectionGapClassName)}>
            <PageHeader
              leading={
                <Button asChild variant="ghost" size="sm" className="-ml-1 mb-0.5 h-7 px-2 text-xs">
              ...
              title={
                <span className="inline-flex flex-wrap items-center gap-1.5">
                  {collection.name}
                  {shouldShowCollectionStatus(collection.status) ? (
                  ...
                  <CardContent>
                    <ul className="divide-y text-sm">
                      {validation.issues.map((issue, index) => (
                        <li key={`${issue.message}-${index}`} className="px-0.5 py-1.5">
            ...
            <Card>
              <CardHeader className="flex flex-row items-center justify-between gap-1">
                <CardTitle>{t('collections.itemsTitle')}</CardTitle>
                <Button
                  size="sm"
                  className="h-7"
