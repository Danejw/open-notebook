...
**Goal:** Replace-in-place AG-UI event streams for project chat, source chat, and Ask via `ag-ui-langgraph`, with thin domain adapters and existing React UI.
...
- [x] Hooks: `useProjectChat`, `useSourceChat`, `useAsk`
- [x] UI status line + `agentSteps` en-US i18n

### Manual verification

- [ ] Project chat: step status + assistant text (or refetch after `RUN_FINISHED`)
