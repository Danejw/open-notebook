...
**Goal:** Ship Streamable HTTP MCP client connections, tool discovery, chat tool selection/execution (read-only), audit history, and Tools UI for notebook + source chat.

**Architecture:** Shared `open_notebook/mcp/` domain (transport, SSRF, risk, allowlist, discovery, execution); SurrealDB models; FastAPI `/mcp/*`; LangGraph `bind_tools` loop in `chat.py` / `source_chat.py`; AG-UI `TOOL_CALL_*`; Tools list/detail + `ToolPicker`.
...
| Path | Role |
|------|------|
| `open_notebook/mcp/limits.py` | Central limits/constants |
| `open_notebook/mcp/url_safety.py` | URL + SSRF validation |
| `open_notebook/mcp/risk.py` | Risk classification |
| `open_notebook/mcp/result_text.py` | Bound result text for model |
| `open_notebook/mcp/schema_validate.py` | Argument schema validation |
| `open_notebook/mcp/transport.py` | Streamable HTTP JSON-RPC + SSE parse |
| `open_notebook/mcp/client.py` | Session facade |
| `open_notebook/mcp/public.py` | Safe public dict builders |
| `open_notebook/mcp/discovery.py` | Tool sync |
| `open_notebook/mcp/allowlist.py` | Runtime allowlist + aliases |
| `open_notebook/mcp/execution.py` | Audit + call helpers + duplicate guard |
| `open_notebook/mcp/langgraph_tools.py` | Build LangChain tools from allowlist |
| `open_notebook/domain/mcp.py` | McpConnection, McpTool, ChatToolCall |
| `open_notebook/database/migrations/18.surrealql` | Schema |
...
| `open_notebook/graphs/chat.py` / `source_chat.py` | Tool loop |
...
**Files:** `open_notebook/mcp/{limits,url_safety,risk,result_text,schema_validate,public}.py`, `tests/test_mcp_url_safety.py`, `tests/test_mcp_risk.py`, `tests/test_mcp_result_text.py`, `tests/test_mcp_schema_validate.py`
...
- [ ] URL safety: http/https only; reject userinfo; block private/loopback/link-local/metadata unless `OPEN_NOTEBOOK_MCP_ALLOW_PRIVATE_URLS=true`
...
**Files:** `open_notebook/mcp/transport.py`, `client.py`, `tests/fixtures/fake_mcp_server.py`, `tests/test_mcp_transport.py`
...
**Files:** `open_notebook/domain/mcp.py`, `migrations/18.surrealql`, `18_down.surrealql`, `async_migrate.py`, register subclasses for polymorphic get
...
**Files:** `open_notebook/graphs/chat.py`, `source_chat.py`, `api/routers/chat.py`, `source_chat.py`, `ag_ui_agents.py`, prompts if needed
...
**Files:** `ToolPicker.tsx`, `ToolCallCard.tsx`, `useNotebookChat.ts`, `useSourceChat.ts`, `ChatColumn`, `ChatPanel`, AG-UI event handlers
