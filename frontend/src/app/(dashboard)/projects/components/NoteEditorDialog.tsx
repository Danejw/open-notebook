...
export function NoteEditorDialog({ open, onOpenChange, projectId, note }: NoteEditorDialogProps) {
  ...
  const onSubmit = async (data: CreateNoteFormData) => {
    if (note) {
      ...
      // Only invalidate notebook-specific queries if we have a projectId
      if (projectId) {
        queryClient.invalidateQueries({ queryKey: QUERY_KEYS.notes(projectId) })
      }
    } else {
      // Creating a note requires a projectId
      if (!projectId) {
        console.error('Cannot create note without project_id')
        return
      }
