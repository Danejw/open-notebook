...
> **Credential System**: Construction OS uses encrypted credentials stored in the database. Each credential connects to a provider and allows you to discover, register, and test models.

---

## Overview

Construction OS manages AI provider access through a **credential-based system**:
...
Add `CONSTRUCTION_OS_ENCRYPTION_KEY` to your docker-compose.yml:
...
3. The models are now available throughout Construction OS
...
- Keep `CONSTRUCTION_OS_ENCRYPTION_KEY` — it's still required
...
|---------|---------------|---------------------------|
| Password | `construction-os-change-me` | Set `CONSTRUCTION_OS_PASSWORD` |
| Encryption Key | None (must be set) | Set `CONSTRUCTION_OS_ENCRYPTION_KEY` to any secret string |
...
| Error on save | Encryption key not set | Set `CONSTRUCTION_OS_ENCRYPTION_KEY` in docker-compose.yml |
